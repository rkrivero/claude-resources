# Banco de casos resueltos — análisis espacial cuantitativo

Casebook aplicado con transformaciones antes/después para investigación
cuantitativa espacial. Cada caso integra las dos capas: corrección del
castellano académico y rigor APA 7.
...
> Habiéndose determinado que la heterogeneidad espacial del área de estudio,
> la cual influye en la accesibilidad a través de las barreras físicas
> existentes, genera una distorsión que afecta negativamente los valores de
> la variable dependiente, se procedió a la aplicación de un modelo GWR con
> el objeto de evaluar el impacto real en el área de estudio.

✔️ **Corregido (3 unidades, orden S+V+C):**
> La heterogeneidad espacial del área de estudio restringe la accesibilidad
> a través de las barreras físicas presentes. Esta distorsión geográfica
> sesga los valores de la variable dependiente. Para cuantificar este impacto,
> aplicamos un modelo de regresión geográficamente ponderada (GWR).
...
> En las zonas periféricas del área de estudio, debido a la falta de
> infraestructura y la presencia de barreras geográficas restrictivas, de manera
> muy drástica la variable de respuesta cae.

✔️ **Corregido (focalización inicial, 25 palabras):**
> La variable de respuesta desciende en la periferia del área de estudio.
> Esta contracción responde a las barreras geográficas y a la carencia de
> infraestructura básica.

**Qué se corrigió:** circunstanciales movidos al final; núcleo (`la variable… cae`)
...
> Un factor a tener en cuenta en el tema del análisis es que la variable
> accesibilidad tiene una característica que produce un efecto positivo en la
> cuestión del valor del indicador de interés.

✔️ **Corregido (12 palabras):**
> La accesibilidad eleva significativamente el valor del indicador de interés.
...
✔️ **Corregido (bloque homogéneo, 4 líneas, 50 palabras):**
> Los modelos de mínimos cuadrados ordinarios (OLS) asumen homogeneidad espacial.
> Sin embargo, en presencia de dependencia espacial, los residuos de estas
> estimaciones globales suelen presentar autocorrelación. Este fenómeno infringe
> los supuestos clásicos de la econometría y sesga los coeficientes estructurales.
> En consecuencia, es indispensable transicionar hacia modelos de regresión espacial.

**Qué se corrigió:** fusión en una unidad temática; marcadores explícitos al
inicio de oración (`Sin embargo`, `En consecuencia`); contextualización al
fenómeno observado en los datos.
...
> la sección de resultados intentó demostrar cómo las variables predictoras
> juegan un rol destructivo en la variable dependiente.

✔️ **Corregido (27 palabras, voz activa institucional):**
> Utilizamos el modelo SAR/SEM para analizar la estructura de los errores
> espaciales. Los resultados demuestran que las variables predictoras de
> localización reducen significativamente la variable de respuesta.
