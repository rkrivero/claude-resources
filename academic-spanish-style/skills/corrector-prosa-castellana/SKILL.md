---
name: corrector-prosa-castellana
description: >
  Skill especializado en vicios del español académico basado en Fundéu, RAE y
  Martínez de Sousa. Disparadores: "suena torpe / pesado / cargado", "tiene
  ...
  castellano, sintaxis y cohesión, lenguaje libre de sesgos. Complementa
  `diagnostico-textual` (capas de coherencia/adecuación) y `revision-por-capas`
  (Pase 2 normativa) de este mismo plugin; también complementa al
  `scholarly-writing-coach` (apa7-guidelines) para diagnóstico APA 7.
metadata:
  version: "1.1.0"
...
Para transformaciones modelo (antes/después) aplicadas a investigación cuantitativa
espacial, consultar `references/banco-casos.md`: cinco casos resueltos que cubren
frases-cascada,
...
"El análisis de regresión espacial que se llevó a cabo mediante el software
 ...
 orden, estandarizada por fila, reveló que la accesibilidad era la
 variable más influyente en la determinación de la variable dependiente."

Dividida:
"Se estimó una regresión espacial en Python (spreg) con matriz queen de
 primer orden, estandarizada por fila. La accesibilidad resultó ser
...
```
Problemático:
"Las unidades espaciales ubicadas en la zona de alta densidad del área de
 estudio, clasificadas según criterios de uso de suelo definidos por la
 normativa vigente y verificados mediante trabajo de campo, presentan
 valores un 38% superiores al promedio."

Mejor (inciso al final):
"Las unidades de la zona de alta densidad presentan valores un 38%
 superiores — clasificadas según la normativa de uso de suelo vigente."
```
...
```
Mal:  "El modelo estima pura y simplemente la varianza espacial residual."
Bien: "El modelo estima pura y simplemente la varianza espacial residual."
...
```
Núcleo tardío:  "En el contexto del análisis realizado, la accesibilidad
                 resultó la variable más influyente."
Núcleo temprano: "La accesibilidad fue la variable más influyente."
```
...
Mal:  "Actualmente, ciertamente, el método estadístico espacial funciona eficientemente."
```
...
| los ciudadanos del área de estudio | la ciudadanía local · los habitantes de la zona |
...
**Integración dentro de este plugin:**
- `diagnostico-textual` — diagnóstica niveles coherencia/adecuación (Cassany); este skill profundiza en la capa normativa/cohesión con etiquetas RAE/Fundéu.
- `revision-por-capas` Pase 2 — el Pase 2 delega normativa a este skill para el checklist detallado.
- `legibilidad-estilo-llano` — transforma la prosa densa; este skill corrige los vicios gramaticales específicos del castellano.

**Integración cross-plugin con apa7-guidelines:**
El `scholarly-writing-coach` cubre APA §4-6 (voz, tiempos verbales, mecánica estadística, prosa LLM); este skill cubre los vicios propios del castellano. Se pueden activar en secuencia: APA primero, castellano después.
